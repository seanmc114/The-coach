...
function presentCoachModal(){
  try{
    ...
    const html = `
      ...
      <div class="muted" style="margin-top:10px">Task = what the examiner wants. Your line = what you typed. Fix ONE thing and go again.</div>
...
const hasConnectorES = (s)=> /\b(y|pero|porque|además|entonces|también|sin\s+embargo|ya\s+que)\b/i.test(String(s||""));
  ...
  function wrongsAllowed(level){
    const lvl = Math.max(1, Math.min(10, Number(level)||1));
    // Keep early levels forgiving; mastery still tight later.
    // L1:5, L2:4, L3:4, L4:3, L5:3, L6:2, L7:2, L8:1, L9:1, L10:1
    if(lvl<=1) return 5;
    if(lvl<=3) return 4;
    if(lvl<=5) return 3;
    if(lvl<=7) return 2;
      ...
      items.forEach(it=>{
        ...
        card.innerHTML = `
          <div class="fbTop">
            ...
            <div class="fbVerdict ${it.ok?"good":"bad"}">${it.ok?"OK":"Fix"} · ${(()=>{ const n=Number(it.score10); return Number.isFinite(n)?Math.round(n):""; })()}/10</div>
          </div>
          <div class="fbMeta">
            <div class="fbScore">${(()=>{ const n=Number(it.score10); return Number.isFinite(n)?(`${Math.round(n)}/10`):""; })()}</div>
