...
function presentCoachModal(){
  try{
    ...
    const fixLine = String(wrongItem.fix||"").trim();
    const fixHtml = (()=>{
      if(!fixLine) return "";
      const raw = String(fixLine).replace(/^•\s*/,"");
      const parts = raw.split(/\s*\|\s*•\s*/).map(s=>s.trim()).filter(Boolean);
      return `<div class="coachFix">${parts.map(x=>`<div>• ${escapeHtml(x.replace(/^•\s*/,""))}</div>`).join("")}</div>`;
    })();
    ...
    const html = `
      ...
      <div class="muted" style="margin-top:8px"><b>Round score:</b> ${(()=>{const pts=(m.items||[]).reduce((a,it)=>a+(Number(it.score10)||0),0); const pct=Math.round((pts/(PROMPTS_PER_ROUND*10))*100); return pts + "/" + (PROMPTS_PER_ROUND*10) + " (" + pct + "%)";})()} &nbsp;•&nbsp; <b>Time:</b> ${fmtTime((m.scoreSec||0)*1000)} (includes +${PENALTY_SEC}s per wrong)</div>
      ${fixHtml ? `<div class="muted" style="margin-top:10px"><b>Fix ONE thing:</b>${fixHtml}</div>` : ""}
...
function detectTags(prompt, answer, lang, rubric){
  ...
  if(lang==="es"){
    ...
    if(/\bme\s+gosto\b/.test(low)){ tags.push("verb_form"); examples.push("me gosto → me gusta"); }
    ...
    // Singular/Plural mismatch: "es grandes" → "es grande" (or "son grandes" if subject is plural)
    if(/\bes\s+grandes\b/.test(low)){
      tags.push("agreement");
      examples.push("es grandes → es grande");
    }
    // Plural noun with "es": "las clases es..." → "las clases son..."
    if(/\b(clases|aulas|instalaciones|vacaciones|cosas)\b[^.?!]{0,18}\bes\b/.test(low)){
      tags.push("verb_form");
      examples.push("Plural subject: es → son");
    }
    // Feminine noun + adjective ending -o: "mi cama está comodo" → "cómoda"
    if(/\b(cama|habitaci[oó]n|casa|clase|ciudad|tienda|cantina)\b[^.?!]{0,30}\b(es|est[aá]|esta)\b[^.?!]{0,22}\b\w+o\b/.test(low)){
      tags.push("agreement");
      examples.push("Feminine: -o → -a (cómodo → cómoda)");
    }
    // "ella es guapo" style
    if(/\bella\b[^.?!]{0,18}\b(es|est[aá]|esta)\b[^.?!]{0,18}\b\w+o\b/.test(low)){
      tags.push("agreement");
      examples.push("ella ... guapo → guapa");
    }
...
function pickRoundFocus(items, lang, rubric){
  ...
  const labelMap = {
    ...
    word_choice: "Word choice (está bien…)",
  ...
  // IGNORE_DETAIL_WHEN_GRAMMAR: if any real language-cog tag exists, do not let "too_short/detail" win focus.
  const grammarTags = ["wrong_person","off_topic","verb_form","verb_ending","word_order","missing_be","articles_gender","agreement","articles","word_choice"];
  const hasGrammar = grammarTags.some(t=> (counts[t]||0) > 0);
  let tagsList = tags.slice();
  if(hasGrammar){ tagsList = tagsList.filter(t=> t!=="too_short" && t!=="detail" && t!=="ok"); }
  let best = "detail";
  let bestScore = -1;
  tagsList.forEach(t=>{
...
score = Math.max(0, Math.min(10, score));
  ...
  else if(tags.includes("word_choice")) note="Word choice";
...
if(!ans || ans==="—"){
  ...
  // Even short lines can have a clear "big cog" error (agreement / verb form etc). Flag that first.
...
const sub = qualityGood ? `Nice one, ${escapeHtml(name)}.` : `Right ${escapeHtml(name)} — quick fix time.`;
  const html = `
    ...
    ${ (wrote && model) ? `<div class="muted" style="margin-top:8px"><b>Your line:</b> ${wrote}<br><b>Coach wants:</b> ${model}</div>` : "" }
