...
function buildPromptUI(){
  ...
  function handleNext(){
    // Click guard: prevents double-firing on some browsers / mixed bindings
    const now = Date.now();
    if(state._nextClickAt && (now - state._nextClickAt) < 180) return;
    state._nextClickAt = now;
    ...
    finishRound().catch(e=>{ console.error(e); try{ toast('Coach error — try again'); }catch(_){ } state.isMarking=false; try{ setNavLocked(false); }catch(_){ } });
  }
...
if(el.rewardOk){
  el.rewardOk.addEventListener("click", ()=> el.rewardPop && el.rewardPop.classList.add("hidden"));
    ...
    // Hard-bind via onclick as well (helps if other scripts interfere with listeners)
    if(el.prevBtn) el.prevBtn.onclick = (e)=>{ try{ e.preventDefault(); }catch(_){} handlePrev(); };
    if(el.nextBtn) el.nextBtn.onclick = (e)=>{ try{ e.preventDefault(); }catch(_){} handleNext(); };
