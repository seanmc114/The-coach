...
function presentCoachModal(){
  try{
    ...
    const refItem =
      items.find(it=> (it.tags||[]).includes(focusTag) && !it.ok)
      || items.find(it=>!it.ok)
      || items[0]
      || {};
    const focusLabelRaw = m.focusLabel || "One clean improvement";
    const top = Array.isArray(m.topTags) ? m.topTags : [];
    const also = top.filter(t=>t && t.label && t.tag && t.tag!==focusTag && t.tag!=="ok" && t.label!=="Good").slice(0,3).map(t=>t.label);
    ...
    const reasonMap = {
      spelling: "Spelling costs easy points. Accents help for a perfect 10/10 — but you can still pass without them.",
      ...
      word_choice: "Word choice matters: bien/bueno, interesante/aburrido. Use the right word.",
    ...
    const drillMap = {
      ...
      word_choice: "Drill: choose the better word (bien/bueno).",
    ...
    const prompt = String(refItem.prompt||"").trim();
    const ans = String(refItem.answer||"").trim();

const focusName = coachPlainFocus(focusTag, ans, state.lang);
const fixHints = coachFixHints(focusTag, prompt, ans, state.lang).slice(0,3);
    ...
    const html = `
      ...
      <div><b>Today’s focus:</b> ${escapeHtml(focusName)}</div>
      ...
      <div class="muted" style="margin-top:8px"><b>Round score:</b> ${(() => { const pts=(items||[]).reduce((a,it)=>a+(Number(it.score10)||0),0); const pct=Math.round((pts/(PROMPTS_PER_ROUND*10))*100); return pts + "/" + (PROMPTS_PER_ROUND*10) + " (" + pct + "%)"; })()} &nbsp;•&nbsp; <b>Time:</b> ${fmtTime((m.scoreSec||0)*1000)} (includes +${PENALTY_SEC}s per wrong)</div>
      ${also.length ? `<div class="muted" style="margin-top:8px"><b>Also watch:</b> ${escapeHtml(also.join(" · "))}</div>` : ""}
      ${prompt ? `<div class="muted" style="margin-top:10px"><b>Task:</b> ${escapeHtml(prompt)}</div>` : ""}
      ${(ans && ans!=="—") ? `<div class="muted" style="margin-top:6px"><b>Your line:</b> ${escapeHtml(ans)}</div>` : ""}
      ${fixHints && fixHints.length ? `<div class="muted" style="margin-top:10px"><b>Fix:</b> • ${escapeHtml(fixHints.join("  • "))}</div>` : ""}
      <div class="muted" style="margin-top:10px">Task = the question. Your line = your attempt. Fix ONE thing (above) and go again.</div>
      <div class="muted" style="margin-top:8px">Choose your next move.</div>
    `;
...
function detectTags(prompt, answer, lang, rubric){
  ...
  if(lang==="es"){
    ...
    if(/\bme\s+gustan\b/.test(low) && !/\bme\s+gustan\b\s+(los|las)\b/.test(low)){ tags.push("articles"); examples.push("me gustan + plural noun usually needs los/las"); }
    ...
    if(/\bella\s+es\s+\w+o\b/.test(low)){
      tags.push("agreement");
      examples.push("ella es guapo → ella es guapa");
    }
    ...
    // Plural noun + singular "es" (very common): "sus clases es..." -> "sus clases son..."
    if(/\b(clases|aulas|paredes|instalaciones)\s+es\b/.test(low) || /\bsus\s+clases\s+es\b/.test(low)){
      tags.push("verb_form");
      tags.push("agreement");
      examples.push("Plural subject → use son/están (clases es → clases son)");
    }

    // Noun + "eres" used as description (should be "es"): "cama eres cómoda" -> "La cama es cómoda"
    if(/\b(eres)\b/.test(low) && /\b(cama|habitaci[oó]n|clase|aula|instituto|colegio|cocina|ba[nñ]o)\b/.test(low) && !/\b(t[uú]|vosotros|vosotras)\b/.test(low)){
      tags.push("wrong_person");
      tags.push("verb_form");
      examples.push("Describing a thing/person: use es/está (eres → es/está)");
    }

    // Feminine plural subject but masculine plural adjective: "clases son instructivos" etc.
    if(/\b(clases|aulas|paredes)\s+son\s+\w+os\b/.test(low)){
      tags.push("agreement");
      examples.push("clases/aulas/paredes → adjectives in -as (instructivos → instructivas)");
    }

    // Agreement: "las X no son grande" etc (plural adjective needs -s)
    if(/\b(las|mis|sus)\s+\w+s\b[^.?!]{0,30}\b(son|est[aá]n)\s+(grande|interesante|importante|verde|azul)\b/.test(low) && !/\b(son|est[aá]n)\s+\w+s\b/.test(low)){
      tags.push("agreement");
      examples.push("Plural adjective needs -s (grande → grandes)");
    }
    ...
    // Feminine plural agreement (common in JC): clases/aulas/paredes → adjectives in -as
    if(/\b(clases|aulas|paredes)\s+son\s+\w+os\b/.test(low)){
      tags.push("agreement");
      examples.push("Feminine plural: clases/aulas/paredes → adjectives in -as (instructivos → instructivas)");
    }
  ...
  // If there's a real language cog, don't nag for "More detail" as the main issue.
  if(tags.includes("too_short")){
    const real = tags.filter(t=>!["too_short","spelling","ok","detail"].includes(t));
    if(real.length) tags.splice(tags.indexOf("too_short"),1);
  }
...
function focusLabel(tag, lang){
  ...
  if(tag==="word_choice") return "Word choice";
...
function coachPlainFocus(tag, answer, lang){
  const a = String(answer||"").toLowerCase();
  if(tag==="wrong_person"){
    if(/\beres\b/.test(a) || /\bme\s+gustas\b/.test(a)) return "Wrong person / verb";
    return "Wrong person";
  }
  if(tag==="verb_form" || tag==="verb_ending") return "Verb form";
  if(tag==="missing_be") return "Missing ‘to be’";
  if(tag==="agreement" || tag==="articles_gender" || tag==="articles") return "Agreement";
  if(tag==="word_choice") return "Better word";
  if(tag==="word_order") return "Word order";
  if(tag==="spelling") return "Spelling (accents = bonus)";
  if(tag==="too_short") return "Too short";
  if(tag==="detail") return "Add one detail";
  if(tag==="off_topic") return "Off topic";
  return "One clean fix";
}

function coachFixHints(tag, prompt, answer, lang){
  ...
  if(!a || a==="—"){
    return [
      "Write ONE simple sentence (4–8 words).",
      "Use a starter from the prompt helpers.",
      "Then add ONE extra detail (porque / también / a veces)."
    ];
  }

  if(tag==="wrong_person"){
    if(/\beres\b/.test(low)) hints.push("You’re describing he/she: change 'eres' → 'es'.");
    if(/\bme\s+gustas\b/.test(low)) hints.push("It’s 'me gusta' (no -s).");
    if(/\bella\s+es\s+guapo\b/.test(low)) hints.push("ella → 'guapa' (feminine).");
    if(!hints.length) hints.push("Check who you’re talking about (I / you / he-she / we / they). Fix the verb to match.");
  } else if(tag==="verb_form" || tag==="verb_ending"){
    if(/\bme\s+gustas\b/.test(low)) hints.push("It’s 'me gusta' (not 'me gustas').");
    if(/\bes\s+bien\b/.test(low)) hints.push("Say 'está bien' or 'es bueno' (choose one).");
    if(!hints.length) hints.push("Fix the main verb first (soy/tengo/me gusta/voy/hago).");
  } else if(tag==="agreement"){
    if(/\bson\s+grande\b/.test(low)) hints.push("Plural: 'grande' → 'grandes'.");
    if(/sus\s+clases\s+es\b/.test(low) || /\bclases\s+es\b/.test(low)) hints.push("Plural subject: 'es' → 'son'.");
    hints.push("Make endings match: alto/alta, grande/grandes, divertido/divertida(s).");
  } else if(tag==="missing_be"){
    hints.push("Add a ‘to be’ verb: es/está/son/soy…");
    hints.push("Then keep it one clean sentence.");
  } else if(tag==="spelling"){
    if(lang==="es"){
      try{
        const fx = wordFixesES(a);
        const changes = (fx && Array.isArray(fx.changes)) ? fx.changes.slice(0,4) : [];
        if(changes.length) hints.push("Fix: " + changes.join("  |  "));
      }catch(_){}
      hints.push("Accents help for 10/10 — but don’t block your pass.");
    } else {
      hints.push("Fix the obvious spelling errors first.");
    }
  } else if(tag==="word_choice"){
    hints.push("Choose the stronger word: 'bueno' vs 'bien' (adjective vs adverb).");
    hints.push("Keep the sentence clean.");
  } else if(tag==="too_short"){
    hints.push("Add ONE detail (where / with who / why / how often).");
    hints.push("Keep it one sentence.");
  } else if(tag==="detail"){
    hints.push("Add ONE extra detail (porque / también / a veces / normalmente).");
    hints.push("One sentence only.");
  } else if(tag==="off_topic"){
    hints.push("Answer THIS prompt (use 2–3 words from the question).");
    hints.push("Then add one detail.");
  } else {
    hints.push("Fix ONE thing that blocks meaning (verb / agreement).");
  }
...
function tipForTags(tags, lang){
  ...
  if(lang==="es"){
    ...
    if(has("word_choice")) return "Word choice: está bien / es bueno (not *es bien*).";
...
function pickRoundFocus(items, lang, rubric){
  ...
  const labelMap = {
    ...
    word_choice: "Word choice (bien/bueno…)",
...
function modelFromPromptES(p, level){
  ...
  if(/sport|deporte|favourite sport|favorite sport|deporte favorito/.test(s)) return "Mi deporte favorito es el fútbol porque es emocionante y juego con mis amigos.";
...
function pickDetailAddonES(p){
  ...
  if(/sport|deporte|favourite sport|favorite sport|deporte favorito/.test(s)) return pick([
    "porque es emocionante",
    "y lo practico con mis amigos",
    "y juego los fines de semana",
    "porque me mantiene en forma",
    "y veo los partidos por la tele"
  ]);
...
const hasConnectorES = (s)=> /\b(y|pero|porque|además|entonces|también|sin\s+embargo|ya\s+que)\b/i.test(String(s||""));
  ...
  function unlockTargetForLevel(level){
    ...
    // Unlock schedule (generous): Level 1 unlocks next level under 10:00,
    // then -0:30 per level. (Includes +30s per wrong.)
    return Math.max(240, 600 - (lvl-1)*30);
  }
...
function wrongsAllowed(level){
  const lvl = Math.max(1, Math.min(10, Number(level)||1));
  // More generous early unlocks (free-writing is slower than Turbo verbs):
  // L1:6, L2–3:5, L4–5:4, L6–7:3, L8–9:2, L10:1
  if(lvl<=1) return 6;
  if(lvl<=3) return 5;
  if(lvl<=5) return 4;
  if(lvl<=7) return 3;
...
function buildPromptUI(){
  ...
  const ES_FIX = {"espanol":"español","tambien":"también",
  "encata":"encanta","facil":"fácil","dificil":"difícil","futbol":"fútbol","musica":"música","también":"también"};
...
if(tags.includes("spelling")){
  const otherBad = tags.filter(t=>t!=="spelling" && t!=="ok").length;
  if(otherBad===0){
    // Spelling/accents only: keep it a pass, just prevent a perfect 10.
...
function fallbackScore10(answer, det, rubric){
  const ans = String(answer||"").trim();
  if(!ans || ans==="—") return 0;
  const wc = countWords(ans);
  let score = (wc<4) ? 3 : 7;
  ...
  const tags = (det && det.tags) ? det.tags : [];
  if(tags.includes("blank")) return 0;

  // Penalise real cogs
  const hard = ["wrong_person","off_topic","verb_form","verb_ending","word_order","missing_be"];
  const medium = ["articles_gender","agreement","articles","word_choice"];
  hard.forEach(t=>{ if(tags.includes(t)) score -= 2; });
  medium.forEach(t=>{ if(tags.includes(t)) score -= 1; });

  // Spelling/accents should not block a pass on their own
  if(tags.includes("spelling")){
    const otherBad = tags.filter(t=>t!=="spelling" && t!=="ok").length;
    if(otherBad===0) score = Math.min(score, 9);
    else score -= 1;
  }
...
if(!ans || ans==="—"){
  ...
  const HARD = ["wrong_person","off_topic","verb_form","verb_ending","word_order","missing_be","articles_gender","agreement","word_choice"];
...
const s10 = (jm && Number.isFinite(jm.score10)) ? jm.score10 : fallbackScore10(ans, det, rubric);

items.push({ n:i+1, prompt:p.text, answer:ans||"—", ok, reason, suggestion: coachModel, tip: t2, why, tags: det.tags, examples: det.examples,
             aiFix, score10: s10, checks: jm.checks, missed: jm.missed, earned: jm.earned, note: jm.note });
    }
...
const sub = qualityGood ? `Nice one, ${escapeHtml(name)}.` : `Right ${escapeHtml(name)} — quick fix time.`;
  const html = `
    ...
    ${ (ref && wrote && model) ? `<div class="muted" style="margin-top:8px"><b>Task:</b> ${escapeHtml(ref.prompt||"")}<br><b>Your line:</b> ${wrote}<br><b>Model example:</b> ${model}</div>` : "" }
    <div class="muted" style="margin-top:6px">
      Focus: <b>${escapeHtml(focus)}</b>
      <span class="muted"> • </span>
      Points: <b>${(m.items||[]).reduce((a,it)=>a+(Number(it.score10)||0),0)}/${PROMPTS_PER_ROUND*10}</b>
      <span class="muted"> • </span>
      Time: <b>${fmtTime((m.scoreSec||0)*1000)}</b>
      <span class="muted"> • </span>
      Wrong: <b>${m.wrong}</b>
    </div>
    ${exHtml}
...
function renderResults(){
      ...
      items.forEach(it=>{
        ...
        card.innerHTML = `
          <div class="fbTop">
            ...
            <div class="fbVerdict ${it.ok?"good":"bad"}">${it.ok?"OK":"Fix"} · ${(()=>{ const n=Number(it.score10); return Number.isFinite(n)?Math.round(n):0; })()}/10</div>
          </div>
          <div class="fbMeta">
    ...
    if(el.resultsHint) el.resultsHint.textContent = state.gymRequired ? "Gym is required when you go over the error allowance (or leave blanks). Train, then replay this level — it stays locked until you pass." : "Unlock rule: beat the target time (clock + 30s per wrong) AND stay within the wrong allowance. Points are for bragging rights; time breaks ties.";
  }
...
state.workshop.recentRepSigs = [];
  ...
  function nextGymDrill(){
    // Pick a tag first (for variety), then pick a fresh source item tied to that tag.
...
if(type==="verbs"){
  if(el.wsPrompt) el.wsPrompt.textContent =
    (L==="es") ? pick(["Type ONE clean sentence with a correct verb (soy/tengo/me gusta).","Write ONE sentence using a correct verb form.","One sentence: use a correct verb (me gusta / tengo / soy)."]) :
